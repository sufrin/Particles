...
in lock-step and coordinated by barriers.
  ...
  1. Clicking on a particle increases its density by an order of magnitude (and 
  changes its hue "redward". The mass of a particle is calculated in the usual way
  from its radius, viz: density×(4/3)πR^^3^^

  1. Coefficients of restitution of walls and particles can be set interactively.

  1. δt is the simulated elapsed time between computed frames

  1. FPS is the (target) number of frames to be shown per elapsed second of real time
  This is really here to test the efficiency of our scala code, and under some circumstances 
  it may not be reached; but this doesn't damage the simulation.
...
{ var deltaT     = 3.0        // Time quantum for simulation
  ...
  var Seg       = 1           // Each worker controls (does the calculations for) `2*Seg` particles
  ...
  val Options = List(     OPT("S",      Seg,         s"«S: int» ($Seg) each worker manages 2xS particles ")
                      ,   OPT("P",      P,           s"«P: int» ($P) number of worker processes ")
                      ,   OPT("t",      deltaT,      s"«∂t: double» ($deltaT) time quantum for simulation")
                      ,   OPT("s",      scale,       s"«scale: int» ($scale) order of magnitude of 'gravitational' constant: G = 6.79E<scale>")
                      ,   OPT("W",      wallBounce,  s"«Wall: double» ($wallBounce) When it hits a wall a particle's momentum *= -Wall.")
                      ,   OPT("B",      ballBounce,  s"«Ball: double» ($ballBounce) force multiplier touching particles [negative => repulsion]")
                      ,   OPT("FPS",    FPS,         s"«FPS: int» ($FPS) frames to be shown per second (target value)")
                      ,   OPT("w=",     width,       s"«int» ($width) width of the arena (units)")
                      ,   OPT("h=",     height,      s"«int» ($height) height of the arena (units)")
                      ,   OPT("C",      CF,          s"«CF: double» ($CF) particle speed is limited to |(width/CF∂t, height/CF∂t)|.")
                      ,   OPT("-d",     debug,       s"enable the debugger ($debug)")
                      ,   REST("--",    queueSpecs(_), "Remaining parameters are particle specs of the form <radius>@<x>,<y>")
                    )
  ...
  {
    for (s <- specs) try { ps.enqueue(mkPart(s)) } catch { case e: Exception => println(s"$s is not a valid particle spec")}
  }

  def  mkPart(s: String): Particle =
  { val parts=s.split("[@,]")
    new Particle(parts(0).toDouble, new Position(parts(1).toDouble, parts(2).toDouble))
  }

  /** The indices of the particles owned by process `me` */
  def ownedBy(me: Int): Seq[Int] = ((me * Seg) until ((me + 1) * Seg)) ++ (((2 * P - me - 1) * Seg) until ((2 * P - me) * Seg))

  /** Arbitrary upper bound on present speed of a particle that is to be accelerated: calculated at startup */
  var C: Double             = _
  ...
  /**
    ...
    */
  class Particle(val R: Double, val position: Position, val velocity: Velocity = new Velocity())
  extends Displayable
  { override def toString: String = s"Particle($R, $position, $velocity)"
    ...
    /** Calculate the position and velocity after time deltaT given the total force on the particle */
    def nextState(totalForce: Force): Unit =
    {
      position += velocity * deltaT
      // Bounce off the edges and lose some momentum if necessary
      if (position.x < R + 10 && velocity.x < 0)          { velocity.scaleX(-wallBounce); position.x = R+10 }
      if (position.x + R + 10 > width && velocity.x > 0)  { velocity.scaleX(-wallBounce); position.x = width-R-10 }
      if (position.y < R + 10 && velocity.y < 0)          { velocity.scaleY(-wallBounce); position.y = R+10 }
      if (position.y + R + 10 > height && velocity.y > 0) { velocity.scaleY(-wallBounce); position.y = height-R-10 }
      // limit speed
    ...
    def touches(that: Particle): Boolean = (this.position distanceTo that.position) < (this.R + that.R)
    ...
    /** Particle's redness on the display: determined by its density (up to a point) */
    def color: Color      =
  ...
  { // initialize
    ...
    { // update local state
      ...
      { val p = allParticles(pid)
        ...
        val force = (p attractionTo q) * (if (p touches q) ballBounce else 1.0)
  ...
  /** Whether the simulation is running or not: unsynchronized, but this needn't be atomic */
  var running  = false
      ...
      { case Pressed(mouse)   =>
        {  val lastHits = display.at(mouse.x, mouse.y, running)
           if (!running && lastHits.length==1)
              for (particle <- lastHits) println(s"R=${particle.R}, mass=${particle.mass}")
           ...
           { // remove selection (if selected) or change density
             ...
             for (particle <- lastHits) if (particle.selected) particle.selected=false else particle.changeDensity(factor)
  ...
  { // initialize
    ...
    var repelling  = 1.0
    val gravity    = row(label("6.79E"), spinner(scale, -24, 2, 1){ value => scale=value; G = scaledG()*repelling }.alignLeft) withTitledBorder("Gravity")
    val repel      = col(checkBox("  ", false){ state => repelling = if (state) -1.0 else 1.0; G = math.abs(G)*repelling })    withTitledBorder("Repel")

    val wBounce    = spinner(wallBounce,  -1.0,  5.0,  0.1) { value => wallBounce = value }  withTitledBorder("Wall")
    val bBounce    = spinner(ballBounce, -50.0, 50.0, 0.1)  { value => ballBounce = value }  withTitledBorder("Ball")
    val time       = spinner(deltaT, 0.1, 50, 0.5)          { value => deltaT = value }      withTitledBorder("∂T")
    val fps        = spinner(FPS, 0, 600, 10)               { value => FPS = value+1 }       withTitledBorder("FPS")

    val run        = checkBox("Run", running){ state => running = state; if (running) singleFrame.release() }
    def controls   = row(run, hGlue, label(s"$P workers, $N particles"), hGlue, fps, hGlue, wBounce, bBounce, gravity, repel, time, hGlue).withEtchedBorder()

    display = new Display[Particle](allParticles, title="Particles", width=width, height=height, events=fromDisplay, keys=fromDisplay, North=controls)
    barrier.sync()

    while (true)
       {
         display.draw()           // drawing the frame overlaps with local updates
         singleFrame.acquire()    // wait here for next frame permission, so last draw is completed when `!running`
  ...
  { if (debug) println(debugger)
    ...
    if (ps.isEmpty)
    {
      ...
      //
      for (i<-0 until N)
          allParticles(i) = if (ps.isEmpty) new Particle(10+i, new Position(math.random()*width, math.random()*height)) else ps.dequeue()
      //
      val workers = ||(for (me<-0 until P) yield worker(me, ownedBy(me)))
      run(workers || displayController || mouseManager)
  }
 }
}
