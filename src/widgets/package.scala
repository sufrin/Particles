...
/**
  The classes of this package provide a small selection of widgets derived from `javax.swing` and set up for easy declarative
  specification of simple Interfaces. There are two main differences:

    1. The widgets have a number of convenience methods that
    permit various ''optional'' attributes to be set as they
    are constructed.  This replaces the imperative style of
    specifying optional attributes.
  
    1. Java-style "Listeners" are not used. Instead
    ''action'' widget classes (those that register ActionListeners
    or ChangeListeners) are either specified by an Act (an
    abstract class that must define an `action:Unit=>Unit`
    method at the point of construction, or earlier by
    subclassing) or (more conveniently) accept a suitably-typed
    action method as a parameter when they are constructed.

  We make no pretence of having produced a complete toolkit;
  simplicity has been our watchword.  Moreover, we have -- through
  haste not conviction -- taken a somewhat cavalier approach to the
  subtleties of the Scala type system -- which is changing somewhat
  erratically between versions as we write.
  
  Bernard Sufrin, Oxford, 2008
  */
...
package object widgets
{
  ...
  { val self = this.asInstanceOf[T]
    ...
    def withAlignmentX(f: Double): T =
    { setAlignmentX(f.floatValue); self}

    /** Set the X alignment (for use in Col layouts), and return self */
    def alignX(f: Double): T =
    { setAlignmentX(f.floatValue); self}

    /** Set the Y alignment (for use in Row layouts), and return self */
    def withAlignmentY(f: Double): T =
    { setAlignmentY(f.floatValue); self}

    /** Set the Y alignment (for use in Row layouts), and return self */
    def alignY(f: Double): T =
    { setAlignmentY(f.floatValue); self}

    /** Set the preferred width and height from <code>w, h</code>;
        if either are negative use the existing value
      */
    def withPreferredSize(w: Int, h:Int) : T =
    { setPreferredSize(
                        new Dimension(if (w<0) getPreferredSize.width else w,
    ...
    /** Set the border and return self
        (inadvisable for most non-container components)
      */
    def withBorder(border: Border): T  =
    { setBorder(border); self }

    /** Set an etched border and return self
        (inadvisable for most non-container components)
      */
    def withEtchedBorder() : T = withBorder(etchedBorder)

    /** Set an etched and titled border and return self
        (inadvisable for most non-container components)
      */
    def withTitledBorder(title: String) : T  =
      withBorder(BorderFactory.createTitledBorder(etchedBorder, title))

    /** Embed in a component with a titled, etched  border,
        and return that component
      */
    def inFrameTitled(title: String) : JCol =
      new JCol(List(this)).withTitledBorder(title)

    /** Embed in a component with an etched  border,
        and return that component
      */
  ...
  trait   ButtonWidget[+T]
    extends AbstractButton
      with    Widget[T]
  {
    def withSelected(state: Boolean): T = { setSelected(state); this.asInstanceOf[T] }
  }

  /** An ActionWidget must specify a method
      <tt>action:Unit=>Unit</tt> to be called when the widget's
      action is invoked. This trait takes care of the
      details of registering a listener to call this method.
    */
  trait   ActionWidget[T]
    extends Widget[T]
      with    ActionSpec
  {   def addActionListener(listener: java.awt.event.ActionListener): Unit 
  
      locally {
         addActionListener(new java.awt.event.ActionListener()  { def actionPerformed(ev: ActionEvent):Unit = { action() } } ) 
      }
  }
  ...
  class   JCheckBox
    extends javax.swing.JCheckBox
      with    ButtonWidget[JCheckBox]
  { /** A checkbox based on a given ACT */
    def this(act: Act)  = { this(); setAction(act) }
    /** A checkbox whose action method is passed as a parameter.
        The parameter of the action is the state of the checkbox.
      */
      ...
      with    ButtonWidget[JRadioButton]
  {
    /** A checkbox based on a given ACT */
    def this(act: Act)  = { this(); setAction(act) }
    /** A radiobutton whose action method is passed as a parameter.
        The parameter of the action is the state of the checkbox.
      */
    def this(name: String, act: Boolean => Unit) =
    { this()
  ...
  /**
  A menu with the given caption  and the given menu items.
     Within this class we define methods button, checkBox,
     radioButton to construct their MenuItem counterparts
     and also add them to the menu under construction
    */
  ...
  { private val cont = this
    ...
    /** Locally we define button, checkBox, radioButton to be
        constructors for their MenuItem counterparts that
        also add them to the menu under construction
      */
  ...
  /** A menubar with the given menus.
      Within this class the class Menu behaves as JMenu
      does outside, except it automatically adds the menu
      it is constructing to the MenuBar under construction
    */
  ...
  /** A tool bar with the given orientation and the given widgets.
     Within this class we define methods button, checkBox,
     radioButton that construct appropriate widgets
     and  add them to the menu under construction
  ...
  class JSpinner[D](model: javax.swing.SpinnerModel, act: D=>Unit) extends javax.swing.JSpinner(model) with Widget[JSpinner[D]]
  {
    addChangeListener(new ChangeListener {
      override def stateChanged(e: ChangeEvent): Unit = act(model.getValue.asInstanceOf[D])
    })
    def alignLeft:Widget[JSpinner[D]] =
    { import javax.swing.SwingConstants.LEFT
      getEditor match {
        case ed: javax.swing.JSpinner.DefaultEditor => ed.getTextField.setHorizontalAlignment(LEFT)
  ...
  def spinner(strings: Iterable[String])(act: String=>Unit): JSpinner[String] =
    new JSpinner[String](new javax.swing.SpinnerListModel(strings.toArray.asInstanceOf[Array[Object]]), act)

  def spinner(strings: String*)(act: String=>Unit): JSpinner[String] = spinner(strings.toList)(act)

  def spinner(value: Double, min: Double, max: Double, stepSize: Double)(act: Double=>Unit): JSpinner[Double] =
    new JSpinner[Double](new javax.swing.SpinnerNumberModel(value, min, max, stepSize), act)

  def spinner(value: Int, min: Int, max: Int, stepSize: Int)(act: Int=>Unit)  =
    new JSpinner[Int](new javax.swing.SpinnerNumberModel(value, min, max, stepSize), act)

  /** A Swing <code>JTextField</code> with the given name
      and initial text; it invokes its action
      method when Enter is pressed.
    */
  ...
  /**
  A JFrame with a Border layout that exits the
      application when closed. Usage:
      <pre><code>
        new JApplication { North=...; South=... (etc) }
      </code></pre>
      One or more of North, South, East, West, Center must be
  ...
  class JApplication(title: String) extends JFrame(title)
  {
    ...
    { if (North!=null)  add(North,  "North")
      ...
      if (North==null && South==null &&
        ...
        throw new Error("JApplication with null North, South, East, West, and Center")
      super.pack
    }
  }

  /** A Panel with a Border layout. Usage:
      <code>
        new JBorder { north=...; south=... (etc) }
      </code>
  ...
  /** A JSlider with the given name orientation and
      associated range and value.  It invokes its
      <tt>action</tt> method when its value is changed:
      while the slider is being adjusted if the slider is in
      instant mode, otherwise when the mouse button is
      released at the end of an adjustment.  <p> Default
      major ticks are at 10%
    */
   abstract class  JSlider(min: Int, max:Int, initialValue: Int, or: Orientation = Horizontal)
  ...
  /**
  <tt>Cswing</tt> button and menu item components are
        constructed from descriptors -- which are objects of
        class <tt>Act</tt>.  These specify the name of the
        component and the code to be run when the button is
        pressed.
    */
  ...
  { def actionPerformed(ev: ActionEvent): Unit = { action() }
    ...
    /** Set an icon to be placed in components built from
    ...
    /** Set the (initial) selection state of components to be built
         from this and return this
      */
  ...
  /** A group of selectable ButtonWidgets of which at most one can
       be selected at the same time. Objects of this class implement
       the iterable Interface; so they can be used as arguments to
       Col and Row constructors.
    */
  ...
  { add(Box.createVerticalGlue) }
    ...
    def  menuItem(name: String)(act: => Unit) : JMenuItem = new JMenuItem(name, { case () => act })

    def checkBoxMenuItem(name: String)(act: Boolean => Unit) = new JCheckBoxMenuItem(name, act)

    def radioButtonMenuItem(name: String)(act: Boolean => Unit) = new JRadioButtonMenuItem(name, act)

    def checkBox(name: String)(act: Boolean => Unit) = new JCheckBox(name, act)

    def checkBox(name: String, state: Boolean)(act: Boolean => Unit) = new JCheckBox(name, act) withSelected state

    def button(name: String)(act: => Unit) = new JButton(name, { case ()=>act })

    def radioButton(name: String)(act: Boolean => Unit) = new JRadioButton(name, act)

    def radioButton(name: String, state: Boolean)(act: Boolean => Unit) = new JRadioButton(name, act) withSelected state

    def label(text: String, icon: Icon) = new JLabel(text, icon)

    def label(text: String) = new JLabel(text)

    def buttonGroup[W <: ButtonWidget[Any]](buttons: W*) = new ButtonGroup(buttons)

    def slider(min: Int, max:Int, value: Int, or: Orientation = Horizontal)(act: Int=>Unit) =
        new JSlider(min, max, value, or) { def onChange(value: Int) = act(value) }

    def textField(text: String, cols: Int)(act: String => Unit) =
      new JTextField(text, cols)
      { def action() = act(getText)
      }
